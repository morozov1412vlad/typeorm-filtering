...
import { initializeFilterMetadata } from '../metadata';
// Import the FilterMetadataStorage class, not the instance directly for DI
import { filterMetadataStorage, FilterMetadataStorage } from '../metadata';
...
export class FilterModule implements OnModuleInit {
  ...
  static forRoot(filterClasses: ClassConstructor<any>[]): DynamicModule {
    ...
    const filterServiceProviders = filterClasses.map((filterClass) => {
      return {
        ...
        useFactory: (dataSource: DataSource, metadataStorage: FilterMetadataStorage) => {
    ...
    return {
      module: FilterModule,
      providers: [FilterService, metadataStorageProvider, ...filterServiceProviders],
      exports: [
        FilterService,
  ...
  async onModuleInit() {
    ...
    initializeFilterMetadata(registeredFilterClasses, this.dataSource);
